...
//   //渲染数据
//   function render() {
//     cateData = JSON.parse(localStorage.getItem('pyg_getData'));
...
//         cateData = { 'list': res.Data, 'time': Date.now()}
//         localStorage.setItem('pyg_getData', JSON.stringify(cateData));
//         leftCateList();
//         rightCateList(0);
//       }
//     }, 'json');
...
//     var html = template('leftcateList', cateData);
//     console.log(html);
//     $('.left ul').html(html);
...
//     var righthtml = template('rightcateList', cateData.list[index]);
...
$(function () {
  ...
  // 动态生成右侧二级分类数据
  function rightCateList(index) {
    var html = template('rightcateList', cateData.list[index])
    // 生成了图片的动态结构
    $('.rightList').html(html)
